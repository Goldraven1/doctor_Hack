...
login_manager = LoginManager(app) # это позволяет нам использовать сущность ЮЗЕРА для работы с пользотелями
...
def logout(): # функция лог аут
    logout_user() # непостредственный лог аут из системы
...
def admin():  # функция админ
    return render_template('admin.html', title='Admin Page')  # возвpaт шаблона 
...
def hr_portal(): # функция кадрового работника
...
@login_required # обязательное присутствие авторизации в сервисе
...
def add_user(): # функция добавления юзера
    ...
    return render_template('add_user.html') # ворврат страницы адд_юзер
